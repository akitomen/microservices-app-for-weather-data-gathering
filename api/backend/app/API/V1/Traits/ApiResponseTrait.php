...
trait ApiResponseTrait
{
    ...
    public function parseGivenData($data = [], $statusCode = 200, $headers = [])
    {
        ...
        if (empty($responseStructure['information'])) {
            $responseStructure['information']['timestamp'] = Carbon::now();
            if ($user = auth('api')->user()) {
                $responseStructure['information']['user'] = $user->only(['id', 'name']);
                try {
                    $payload = auth('api')->payload();
                    $responseStructure['information']['ttl'] = $payload['exp'];
                } catch (\Exception $e) {}
            }
        }
